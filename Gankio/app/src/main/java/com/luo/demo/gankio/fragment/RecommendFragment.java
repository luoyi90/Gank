 ...
 */
public class RecommendFragment extends LazyBaseFragment implements LoadMoreScrollListener.LoadMoreListener, SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mLoadingLayout = (LoadingLayout) mRootView.findViewById(R.id.recommend_loadinglayout);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        // KLog.d("recommend onActivityCreated");
    }
    ...
    private void getData() {
        ...
        Api.getInstance().getRecommend(mPageCount, mCurrentPage, new CallBack<Recommend>() {
            @Override
            public void onFinish(final boolean isSuccess, final Recommend bean, final String error) {
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (isSuccess) {
                            ...
                            mData = DataSupport.where("flag=?", "recommend").order("createdAt desc").limit(10).find(ResultsBean.class);
                            if (mData.isEmpty()) {
    ...
    public void onLoadMore() {
        ...
        Api.getInstance().getRecommend(mPageCount, mCurrentPage, new CallBack<Recommend>() {
            @Override
            public void onFinish(final boolean isSuccess, final Recommend bean, String error) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        if (isSuccess) {
                            ...
                            List<ResultsBean> mList = DataSupport.where("flag=?", "recommend").order("createdAt desc")
     ...
     * @param bean
     */
    private void saveAndformat(Recommend bean) {
        // 格式化时间
        for (int i = 0; i < bean.getResults().size(); i++) {
            ...
            bean.getResults().get(i).setFlag("recommend");
        }
